...
<body>
    <main>
        ...
        <form action="inserir_categoria_sql.php" method="post">
            <label>Nome:</label>
        ...
        <table>
            ...
            if ($executar) {
                while ($res = mysqli_fetch_array($executar)) {
                    $id = $res['id_cat'];
                    echo "<tr><td>".$res['id_cat']."</td><td>".$res['nome_cat']."</td><td><a href='del.php?id=$id'>excluir</a></td></tr>";
                }
            } 
            mysqli_close($conexao); // Feche a conexão
            ?>
        </table>
    </main>
</body>
